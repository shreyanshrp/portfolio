...
export function DetailedExperienceDialog() {
  ...
  // Function to handle direct link opening
  const checkUrlAndOpen = () => {
    if (typeof window !== "undefined") {
      const hash = window.location.hash
      if (hash === "#nutanix-detailed") {
        setOpen(true)
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener("hashchange", checkUrlAndOpen)
  }, [])

  return (
    <>
      <Button variant="outline" size="sm" className="rounded-full" onClick={() => setOpen(true)} id="nutanix-detailed">
